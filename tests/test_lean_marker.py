...
for (const lean of ["left", "center-left", "center", "center-right", "right", "state", "non-us", ""]) {
  out[lean] = [leanMarker(lean, { doc })?.html() || "", leanHit("s_1", lean, doc)?.html() || ""];
}
...
def test_build_markup_matches_the_device_renderer():
    ...
    for lean, (marker, hit) in device.items():
        assert marker_html(lean) == marker, lean
        assert hit_html("s_1", lean) == hit, lean
...
def test_catalog_carries_lean_basis_only_for_sources_with_a_marker():
    ...
    data = catalog(pool)
    assert set(data["lean_basis"]) == {"npr", "al_jazeera"}
...
def test_every_scale_or_state_source_has_a_basis_to_show():
    for s in SOURCES:
        if s.get("lean") in LEAN_SCALE + ("state",):
